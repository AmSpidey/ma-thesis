...
%This is done by comparing a Gaussian distribution-based Transformer with one used to predict parameters of a categorical distribution.
\end{abstract}
...
%Transformer proposed in \cite{enhancing} is centered around the idea of every point in a time series being drawn from some Gaussian distribution. Therefore the model's goal is to predict the parameters of these distributions, thus ensuring that each prediction is partially random and introducing Gaussian noise to it. \pk{formally?} \mn{I wanted to leave the formal definitions for the background, is that ok?} 
...
%The goal of our project was using the Transformer model to forecast time series assuming that each point in the sequences comes from a categorical distribution.
...
%This is made feasible by the fact of the Transformer's success in tasks requiring prediction of thousands categories like language translation
...
Since the number of categories we use is finite, serialization is bound to lose the precise value of some data points in a series. With enough buckets used, we hope this drop in precision should be negligible in comparison to our gains in the prediction's reliability. %Furthermore, the anticipated drop in precision can be resolved by 
...
%
%In our preliminary analysis we decided to investigate whether the datasets commonly used in 
...
%We also did not use the risk metric as the loss function, which might have impacted the model's performance negatively.
% All these differences should not have contributed to the gap between Gaussian and Serial Predictors, though.
...
%	\begin{center}
%		\begin{tabular}
%			{ |p{2cm}|p{1.5cm}|p{1.5cm}||p{1.5cm}|p{1.5cm}|  }
%			\hline
%			& \multicolumn{2}{c||}{weighted sampling} & \multicolumn{2}{|c|}{uniform sampling} \\
%			\hline
%			& \hfil $R_{0.5}$ & \hfil $R_{0.9}$ & \hfil $R_{0.5}$ & \hfil $R_{0.9} $
%			\\
%			\hline
%			electricity & \hfil 0.081   & \hfil 0.042    & \hfil \textbf{0.069} &   \hfil \textbf{0.041} \\
%			traffic &  \textbf{\hfil 0.122} &   \hfil \textbf{0.093}   & \hfil 0.151 &   \hfil 0.109\\
%			\hline
%		\end{tabular}
%		\caption{\label{tab:results}Comparison of best performing Serial Predictor models with and without weighted sampling.}
%	\end{center}
%\end{table}
